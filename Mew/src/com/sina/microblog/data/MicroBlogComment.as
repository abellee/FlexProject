package com.sina.microblog.data
{
	...
	public class MicroBlogComment
	{
		 ...
		 * @private
		 */ 
		public function MicroBlogComment()
		{
			
		}
		public function serialization(comment:XML):void
		{
			id = String(comment.id);
			createdAt = MicroBlogDataUtil.resolveDate(comment.created_at);
			text = comment.text;
			user = new MicroBlogUser();
			user.serialization(comment.user[0]);
			
			if ( comment.child("status").length() > 0 )
			{
				status = new MicroBlogStatus();
				status.serialization(comment.status[0]);
			}
			...
			if ( len > 0 )
			{
				...
				{
					replyComment = new MicroBlogComment();
					replyComment.serialization(comment.reply_comment[i]);
