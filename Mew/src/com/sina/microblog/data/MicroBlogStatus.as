package com.sina.microblog.data
{
	...
	public class MicroBlogStatus
	{
		 ...
		 * @private
		 */ 
		public function MicroBlogStatus()
		{
			
		}
		public function serialization(status:XML):void
		{
			id = String(status.id);
			...
			if ( status.child("user").length() > 0 )
			{
				...
				user.serialization(status.user[0]);
			}
			if ( status.child("retweeted_status").length() > 0 )
			{
				repost = new MicroBlogStatus();
				repost.serialization(status.retweeted_status[0]);
			}
		}
	}
}